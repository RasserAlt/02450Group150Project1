...
def hist_plot(Xy, attribute_names):
    ...
    fig.suptitle('Probability density of continuous attributes', fontsize=12)
    plt.show()
    return None
...
def box_plot(Xy,attribute_names):
    ...
    # Plotting Boxplot for each continuous attribute
    for i in range(len(attribute_names)):
        sub_fig = sub_figs[i]
        sub_fig.boxplot(Xy[:,i])
        sub_fig.set_title(attribute_names[i])
    plt.suptitle('Abalone continuous attributes - Boxplot', fontsize=12)
    plt.show()
