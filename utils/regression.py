...
def regression_linear(X, y, variable_names):
    N, M = X.shape
    y = y.squeeze()
    ...
    for m in range(M):
        print('{:>15} {:>15}'.format(variable_names[m], np.round(w_rlr[m, -1], 2)))

    print('Ran Exercise 8.1.1')


def regression_logistic(X, y, variable_names, class_names):
    N, M = X.shape
    C = len(class_names)
